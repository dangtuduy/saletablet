...
class VS_SaleController extends CI_Controller {
    ...
    * Reports
    */
    protected function revenue_current()
    {
        $get = getURLString();
        $todate = date('Y-m-d');
    ...
    protected function revenueByMthStatus()
    {
        $get = getURLString();
    ...
    protected function revenueAllCust()
    {
        $get = getURLString();
    ...
    protected function revenueOneCust()
    {
    ...
    * Select data as list
    */
    protected function viewDetailPerAxOrder($view_sub)//ajax
    {
        $get = getURLString();
        $oid = isset($get['oid']) ? $get['oid'] : '';
        $refoid = isset($get['refoid']) ? $get['refoid'] : '';
        $this->body['dispatch'] = $this->getAPIDispatchJson(getDispatchURL('so', array('id'=> $oid), getWarehouse()));
        //print(getDispatchURL('so', array('id'=> $oid), getWarehouse()));
        $this->body['allOrder'] = $this->salesdb->getAxSalesDetail(DATA_ALL, array('srso'=>$oid, 'refoid'=>$refoid));
        $this->load->view($view_sub, $this->body);
    }

    protected function viewItemList($view_main, $view_sub = '')
    {
        $get = getURLString();
    ...
    protected function viewCustomerList($view_main, $view_sub = '')
    {
        $get = getURLString();
        $curentPage = (isset($get['page']) && $get['page'] > 0) ? $get['page'] : 1;
        $search = array();
        ...
        $resultCust = $this->salesdb->getCustomerByFields($search, ($curentPage-1)*PAGE_ITEM, PAGE_ITEM);
        $arrCust = array();
        foreach ($resultCust as $cust) {
            $arrCust[] = $cust['custid'];
        }
        $this->body['allCust'] = $resultCust;
        $this->body['custloving'] = $this->salesdb->getCustomerLoving($search, $arrCust);
        $this->data['content'] = $this->load->view($view_sub, $this->body, TRUE);
        
        $this->load->view($view_main, $this->data);
    }

    protected function viewAXOrderList($view_main, $view_sub = '')
    {
        $get = getURLString();
        $curentPage = (isset($get['page']) && $get['page'] > 0) ? $get['page'] : 1;
        $this->body['curpage'] = $curentPage;
        ...
        $keyword['srwh'] = isset($get['srwh']) ? $get['srwh'] : getWarehouse();
    ...
    protected function viewAXDetail($view_main, $view_sub = '')
    {
        $get = getURLString();
        $curentPage = (isset($get['page']) && $get['page'] > 0) ? $get['page'] : 1;
        $this->body['curpage'] = $curentPage;
    ...
    protected function viewFOCItemList($view_main, $view_sub = '')
    {
        $get = getURLString();
        $curentPage = (isset($get['page']) && $get['page'] > 0) ? $get['page'] : 1;
        $this->body['curpage'] = $curentPage;
    ...
    protected function viewOrderlist($view_main, $view_sub = '')
    {
        $userid = getUserId();
        $utype = getUserType();
        if($utype != USER_SALEMAN)
    ...
    protected function viewSaleHistory($view_main, $view_sub)
    {   
        $get = getURLString();
        
        $search = array();
        
        $search['srcust'] = VSdecode(isset($get['srcust']) ? $get['srcust'] : '');
        if(empty($search['srcust']))
            $search['srcust'] = isset($get['srcusta']) ? $get['srcusta'] : '';
        
        $search['sritem'] = isset($get['sritem']) ? $get['sritem'] : '';

        $saleman = (getUserType() == USER_SALEMAN) ? getSaleCode() : DATA_ALL;
        $search['srwarehouse'] = (getUserType() == USER_SALEMAN) ? getWarehouse() : '';

        $page = isset($get['page']) ? $get['page'] : 1;

        $list = $this->salesdb->getAxSalesDetail($saleman, $search, ($page-1)*2*PAGE_SEARCH, 2*PAGE_SEARCH);
        $this->body['orderlist'] = $list;
        $this->body['search'] = $search;
        $this->body['curpage'] = $page;
        $this->data['content'] = $this->load->view($view_sub, $this->body, TRUE);

        $this->load->view($view_main, $this->data);
    }

    protected function itemBrand()
    {
    ...
    /*
    * Actions: send email, execute API
    */
    ...
    protected function doResendFile($detail = array())
    {
        if(isset($detail[0]['itemid']) && $detail[0]['itemid'] == 'FILE')
        {
            $first = $detail[0];

            $xmlsend = $this->xmlneworder($OID, $first['custid'], $first['name'], $address.(empty($address2)?'':', @Or: '.$address2), $comment, array());
            
            $attached_file = isset($first['attachedfile']) ? $first['attachedfile'] : '';

            $bodyemail = '<div><b>Customer</b>:'.$first['custid'].' - '.$first['custname'].'<br/><b>Delivery</b>: '.$first['deliverystreet'].'<br/><b>Comment: <span style="color:red">'.$note.'</span></b><br/><b>Sent on:</b>'.(isset($first['lastsend_datetime']) ? $first['lastsend_datetime'] : '').'</div><br/>'.$bodyemail;
                
            $bodyemail = $bodyemail.'<br/><p><b style="color:blue">'.lang('See Attached file'). ' '. $attached_file.'</b></p>';

            $attached_file = realpath(isset($file['file_path']) ? $file['file_path'] : FCPATH.'uploads') .'/'. $attached_file. 
                
            $this->sendEmail($bodyemail, $first['orderid'], 'AGAIN*****'.$first['custid'].' #'.$first['custname'], $attached_file);
        }
    }
    protected function doSendItem2API()//for again
    {
        $get = getURLString();
        ...
        else
        {
            ...
            if(isset($detail[0]['orderid']))
            {
                ...
                if(isset($result['status']) && $result['status'] == TRUE)
                {
                    ...
                    $this->sendEmail($bodyemail, $first['orderid'], 'AGAIN*****'.$first['custid'].' #'.$first['custname']);
    ...
    protected function sendEmail($body, $oid, $customer = '', $attached_file = '')
    {
        ...
        if(isset($email_send[$etype]))
        {
            ...
            if(sendEmailLocal($from_email, $to_mail, $subject, $body, $cc_mail, $attached_file))
    ...
    // GET data from API
    protected function getAPIDispatchJson($apiURL)
    {
        $this->load->library('curl');
