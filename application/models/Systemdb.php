...
class Systemdb extends CI_Model{
    ...
    */
    ...
    function addAxSaleDetailBatch($items = array())
    {
        if(count($items) < 1)
            return FALSE;
    ...
    function addAxHeaderBatch($items = array())
    {
        if(count($items) < 1)
            return FALSE;
    ...
    function addItemCustBatch($items = array())
    {
        if(count($items) < 1)
            return FALSE;
        if($this->db->insert_batch('axitemcustomer', $items))
            return TRUE;
        return FALSE;
    }
    /*
    ...
    */
    ...
    function delAxSaleDetail($fromdate, $todate, $warehouse = 'WMAYW')
    {
    ...
    function delAxSaleHeader($fromdate, $todate, $warehouse = 'WMAYW')
    {
        $this->db->where("(orderdate >='".date('Y-m-d', strtotime($fromdate))."' and orderdate <='".date('Y-m-d', strtotime($todate))."')");
        $this->db->where('warehouse', $warehouse);
    ...
    function delItemCustomer()
    {
        $this->db->empty_table('axitemcustomer');
    }
}
