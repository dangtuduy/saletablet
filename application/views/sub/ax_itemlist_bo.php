<div class="row"><h4 class="title"><b><?=lang('home_item')?> <?=(isset($allItems[0]['created_datetime']) ? '('.$allItems[0]['created_datetime'].')' : '')?></b></h4></div>
    <?php
            $siteurl = site_url('backoffice/itemlist');
            $cn = 0;
    ...
    <div class="row">
        ...
        <?php                           
            foreach($allItems as $item)
            {
                ...
                echo '<td><a href="'.(site_url('sales/focitemlist').'?sritem='.$item['itemid']).'" target="_blank" class="btn btn-primary btntable"><span class="glyphicon glyphicon-th"></span>Foc</a><a href="'.(site_url('sales/instock').'?item='.$item['itemid']).'" target="_blank" class="btn btn-0"><span class="fa fa-list"></span>PO</a>';
                echo '</td></tr>';
            }
            echo '</tbody></table>';
